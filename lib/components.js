...
module.exports = (function () {
  ...
  var generateIndexDefault = function (names, callback) {
    ...
    file.on("error", function (err) {
      console.log((new Date()) + "- ERROR: Write to file error: " + err.toString());
  ...
  var generateSidebarDefault = function (names, callback) {
    var proxyPath = configuration.getConfig().application.proxyPath;
    var outputFileName = configuration.getConfig().customizations.sidebar;
    var file = fs.createWriteStream(configuration.getConfig().application.repository + "/" + outputFileName);
    file.on("error", function (err) {
      console.log((new Date()) + "- ERROR: Write to file error: " + err.toString());
      callback && callback(err);
  ...
  var generateIndex = generateIndexDefault;
  var generateSidebar = generateSidebarDefault;
  ...
  function generateFiles(callback) {
    ...
    readDir(configuration.getConfig().application.repository).then(function (files) {
      ...
      return Promiser.resolve(nameArr);
    }).then(function (names) {
      return new Promiser(function (resolve, reject) {
        // Index  
        if (!configuration.getConfig().customizations.index 
            || configuration.getConfig().customizations.index.length == 0 ) {
          resolve(names);
          return;
        }

        generateIndex && generateIndex(names, function (err) {
          if (err)
            reject(err);
          else
            resolve(names);
      ...
      return new Promiser(function (resolve, reject) {
        // Sidebar
        if (!configuration.getConfig().customizations.sidebar
            || configuration.getConfig().customizations.sidebar.length == 0 ) {
          resolve(names);
          return;
        }

        generateSidebar && generateSidebar(names, function (err) {
          if (err)
            reject(err);
          else
            resolve(names);
  ...
  var publicMethods = {
    ...
    generate: function (cb) {
      generateFiles(function (err) {
        if (!err) {
          ...
          publicMethods.expire("index");
          publicMethods.expire("sidebar");
        }
        cb && cb(err);
      });
    },
    ...
    customIndexGenerator: function (gen) {
      generateIndex = gen;
    },

    customSidebarGenerator: function (gen) {
      generateSidebar = gen;
    }
  };
