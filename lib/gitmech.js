...
function gitSpawn(commands, callback) {
  ...
  child.addListener("close", function () {
    if (exitCode > 0 && stderr.length > 0) {
      ...
      console.log((new Date()) + " - GitSpawn: command[%s], output[%s], error[%s]", commands.join(" "), join(stdout), join(stderr));
    ...
    if (configuration.getConfig().application.loggingMode == 1)
      console.log((new Date()) + " - GitSpawn: command[%s], output[%s]", commands.join(" "), join(stdout));
    else
      console.log((new Date()) + " - GitSpawn: command[%s]", commands.join(" "));
...
function gitExec(commands, callback) {
  ...
  childProcess.exec(commands, { cwd: workTree }, function (error, stdout, stderr) {
    if (error || stderr.length > 0) {
      ...
      console.log((new Date()) + " - GitExec: command[%s], error[%s]", commands, stderr);
      callback(error);
      return;
    }

    if (configuration.getConfig().application.loggingMode == 1)
      console.log((new Date()) + " - GitExec: command[%s], output[%s]", commands, stdout);
    else
      console.log((new Date()) + " - GitExec: command[%s]", commands);
...
var gitMech = {
  ...
  pulling: false, // is running pulling now

  pushing: false, // is running pushing now
  ...
  pull: function (callback) {
    ...
    this.remoteExists(this.remote, function (err, exists) {
      ...
      if (!exists) {
        callback(new Error("Remote does not exist " + "(" + this.remote + ")"));
        return;
      }
      this.pulling = true;
  ...
  createData: function (path, callback) {
    gitSpawn(["log", "--diff-filter=A", "--follow", "--format=%an%n%ae%n%aD", "--", docSubdir + path], function (err, data) {
      if (err) {
        callback(err);
        return;
      }

      var group, arr = [], logdata = data ? data.toString().split("\n") : [];
      // Git command outputs logs in 8 lines per commit.
      for (var i = Math.floor(logdata.length / 3); i-- > 0;) {
        group = logdata.slice(i * 3, (i + 1) * 3);
        arr.push({
          author: group[0],
          email: group[1],
          date: group[2]
  ...
  files: function (callback) {
    gitExec([ "ls-files" ], function (err, data) {
      if (err)
        callback && callback(err);
      else
        callback && callback(null, data.toString().split("\n").filter(function (v) {
          return v !== "";
        }));
    });
  },
  ...
  ls: function (pattern, callback) {
    ...
    gitExec([ "ls-tree", "--name-only", "-r", "HEAD", "--", docSubdir + patt ], function (err, data) {
      if (err) {
        data = "";
      }
      if (isWin) {
